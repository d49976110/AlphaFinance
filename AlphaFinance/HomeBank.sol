...
contract HomoraBank is Initializable, Governable, ERC1155NaiveReceiver, IBank {
    ...
    struct Bank {
        ...
        uint256 pendingReserve; // The pending reserve portion waiting to be resolve.
    ...
    uint256 public feeBps; // The fee collected as protocol reserve in basis point from interest.
    ...
    function resolveReserve(address token) public lock poke(token) {
        Bank storage bank = banks[token];
        require(bank.isListed, "bank not exists");
        uint256 pendingReserve = bank.pendingReserve;
        ...
        returns (address[] memory tokens, uint256[] memory debts)
    {
        ...
        while (bitMap > 0) {
            if ((bitMap & 1) != 0) {
                ...
                Bank storage bank = banks[token];
                tokens[count] = token;
                debts[count] = pos.debtShareOf[token].mul(bank.totalDebt).div(
                    bank.totalShare
                );
                count++;
            }
        ...
        returns (uint256)
    {
        ...
        while (bitMap > 0) {
            if ((bitMap & 1) != 0) {
                ...
                Bank storage bank = banks[token];
                uint256 debt = share.mul(bank.totalDebt).div(bank.totalShare);
    ...
    function execute(
        ...
        address spell,
        bytes memory data
    ) external payable lock onlyEOA returns (uint256) {
        if (positionId == 0) {
        ...
        poke(token)
    {
        ...
        uint256 share = totalShare == 0
            ? amount
            : amount.mul(totalShare).div(totalDebt);
    ...
    function repayInternal(
        ...
        uint256 oldDebt = oldShare.mul(totalDebt).div(totalShare);
    ...
    function putCollateral(
        ...
        if (pos.collToken != collToken || pos.collId != collId) {
            require(
                oracle.support(collToken, collId),
